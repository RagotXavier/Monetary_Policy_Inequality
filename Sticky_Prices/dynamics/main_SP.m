...
if exist(FileNameTot, 'file') == 2
    load(FileNameTot);
    % disp(['File ', FileNameTot, ' loaded successfully.']);
else
...
alpha = alphaa ; % It causes Naming conflicts otherwise 
beta = betaa ; % It causes Naming conflicts otherwise 
gamma = gammaa ; % It causes Naming conflicts otherwise 
...
EcoM = 2; % 2 first economies of the papers
...
for Economy =1:EcoM
    ...
    file = ['code_dynare_',calib,num2str(Economy),'.mod'];
    fid = fopen(file, 'w') ;
    ...
    % LengthSIM = 10000;
    % if bprint
        % str = ['stoch_simul (order=1,irf=',num2str(LengthIRF),',nograph,nofunctions) TFP GDPt Tt Bt PIt rt rnt Lt Ct wt taul tauk Kt Ctot K GDP B Ltot TT BsYt TsYt r RBN PI u ut piw piwt;']; 
    % else
        % str = ['stoch_simul (order=1,irf=',num2str(LengthIRF),', periods=',num2str(LengthSIM),',noprint,nograph,nofunctions) TFP GDPt Tt Bt PIt rt rnt Lt Ct wt taul tauk Kt Ctot K GDP B Ltot TT BsYt TsYt r RBN PI u ut piw piwt;']; 
    % end
    str = ['stoch_simul (order=1,irf=',num2str(LengthIRF),',noprint,nograph,nofunctions) TFP GDPt Tt Bt PIt rt rnt Lt Ct wt taul tauk Kt Ctot K GDP B Ltot TT BsYt TsYt r RBN PI u ut piw piwt;']; 
    ...
    Yv(Economy,:)  = [0 irfs.GDPt_eps];
    Cv(Economy,:)  = [0 irfs.Ct_eps];

    Kv(Economy,:)  = [0 irfs.Kt_eps];
    Lv(Economy,:) = [0 irfs.Lt_eps];

    rnv(Economy,:)= [0 irfs.rnt_eps];
    rv(Economy,:) = [0 irfs.rt_eps];

    Bv(Economy,:)  = [0 irfs.BsYt_eps];
    Tv(Economy,:)  = [0 irfs.TsYt_eps];


    pipv(Economy,:) = [0 irfs.PIt_eps];
    piwv(Economy,:) = [0 irfs.piwt_eps];

    taukv(Economy,:) = [0 100*irfs.tauk_eps];
    taulv(Economy,:) = [0 100*irfs.taul_eps];
    Uv(Economy,:) = [0 irfs.ut_eps];
    ...
    % list = evalin('base', 'oo_.var_list');
    % mean_var = evalin('base', 'oo_.mean');
    % var = evalin('base', 'oo_.var');
    ...
    std_norm = round(std_norm,3); % round for table
    mean_var = round(mean_var,3); % round for table
    ...
    std_norm(index_tauk) = round(std(index_tauk) *100,3);
    std_norm(index_taul) = round(std(index_taul) *100,3);

    % std_norm(index_C) = round((std(index_C)./mean_var(index_C))*100,3);
    ...
    baseFileName_momemt  = sprintf('moments_eco%d', Economy); % Initial file name creation
    ...
    beep;        
        ...
        save(fileName, "rnv", "pipv", "piwv", "Yv", "Bv","Tv", "Cv", "rv","Kv", "taulv", "taukv", "Lv", "Uv", "LengthIRF") 
