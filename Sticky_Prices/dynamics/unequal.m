...
rv = zeros(EcoM,LengthIRF+1); 
rnv = zeros(EcoM,LengthIRF+1); 

Bv = zeros(EcoM,LengthIRF+1);
Tv = zeros(EcoM,LengthIRF+1);

pipv = zeros(EcoM,LengthIRF+1);
...
for Economy =1:EcoM % Loop for the 2 economies
    ...
    % if bprint
        str = ['stoch_simul (order=1,irf=',num2str(LengthIRF),',nograph, noprint,nofunctions) TFP GDPt Tt Bt PIt rt rnt Lt Ct wt taul tauk Kt Ctot K GDP B Ltot TT BsYt TsYt piwt piw PI u ut RBN r; \n']; fprintf(fid, str);
    % else
        % str = ['stoch_simul (order=1,irf=',num2str(LengthIRF),',noprint,nograph,nofunctions,periods=',...
        %     ') TFP GDPt Tt Bt PIt rt rnt Lt Ct wt taul tauk Kt Ctot K GDP B Ltot TT BsYt TsYt piwt piw PI u ut RBN r; \n']; fprintf(fid, str);
    % end
    ...
    var_init = oo_.var;
    autocorr = oo_.autocorr; 


    % A verifier 
    corr_pi_y = var_init(index_Y,index_pip)./( std(index_Y)*std(index_pip) ) ;% Y, pip
    corr_piw_y = var_init(index_Y,index_piw)./( std(index_Y)*std(index_piw) ) ;% Y, pip
    ...
    corr = round([corr_pi_y  corr_piw_y corr_tauk_y  corr_taul_y corr_b_y corr_c_y auto_corr_y_y1 auto_corr_b_b1],2) ;
