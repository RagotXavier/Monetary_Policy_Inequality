    ...
    epss = 6 
    kappa_rob = (epss-1) ./slope_p ;  
       
    
for Economy = 1:length(kappa_rob)

    kappa = kappa_rob(Economy)
    
    kappa_floor = floor(kappa_rob(Economy));
    filename_root = sprintf('todynare_SP_rob_%d', kappa_floor);
    fileExtension = '.mat';
    ...
    params_kappa_verif(1,Economy) = kappa

    index_piw =  find(oo_.var_list == "piw");
    index_pip =  find(oo_.var_list == "PI");

    var_piw  = diag(oo_.var(index_piw)) ;
    var_pip  = diag(oo_.var(index_pip)) ;

    std_piw(1,Economy)  = var_piw.^(1/2) ;
    std_pip(1,Economy)  = var_pip.^(1/2) ;
    mean_piw(Economy) = oo_.mean(index_piw) ;
    mean_pip(Economy) = oo_.mean(index_pip) ;

    Piw_v(Economy,:) =  [0 irfs.piwt_eps] 
    Pip_v(Economy,:) = [0 irfs.PIt_eps ]
