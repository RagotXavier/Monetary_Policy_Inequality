...
calib = 'baseline' ;
    ...
    rv = zeros(EcoM,LengthIRF+1);  % path of real rate
    RBv = zeros(EcoM,LengthIRF+1); % path of nominal rate
    ...
    for Economy = [1, 4]
        ...
        file = ['code_dynare_SW_welfare',num2str(Economy),'.mod'];
        fid = fopen(file, 'w') ;
        formatSpec = '%25.20f';
        ...
        %%%%%%%% FOC planner piw
        if Economy == 4
            str = ['piw = 0;  %% equation ', num2str(equa),'\n']; fprintf(fid,str); equa=equa+1;
        else
        ...
        %%%%%%%% FOC planner pip
        if Economy == 2
            str = ['pip = 0;  %% equation ', num2str(equa),'\n']; fprintf(fid,str); equa=equa+1;
        else
        ...
        %%%%%%%%%%% end of the steady state part
            ...
            str = ['shocks;\n var eps = 0.00001;\nend;\n'];fprintf(fid, str);
            str = ['stoch_simul (order=1,irf=20, periods=',num2str(LengthSIM),',nograph,nomoments,noprint) ;'] ; fprintf(fid, str);
    ...
    Maxseed = 10; 
    Welfare = zeros(Maxseed,4,Ntot); % store the vector of intertemporal utility for each bin 
    ...
    for Economy = [1, 4]
        ...
        for i=1:Maxseed +1 % 
            ...
            toc
            if first_time
                file = ['code_dynare_SW_welfare',num2str(Economy),'.mod',' ','noclearall'];
                command = ['dynare ',file]
                eval(command)
                first_time = 0;
            else
                conso_simul_vec = zeros(Ntot,LengthSIM); % consumption vector by bin and periods
                l_simul_vec = zeros(Ntot,LengthSIM);     % consumption vector by bin and periods
                piw_simul_vec = zeros(1,LengthSIM);      % inflation vector by periods
                ...
                for j=1:Ntot
                    ...
                    idxl = find(strcmp([M_.endo_names], ['L'])) ;
                    conso_simul_vec(j,:) =  oo_.endo_simul(idxc,:) ;
                    l_simul_vec(j,:) =  oo_.endo_simul(idxl,:) ;
                end
                idxpiw = find(strcmp([M_.endo_names], ['piw'])) ;
                piw_simul_vec(:) =  oo_.endo_simul(idxpiw,:) ;

                Ut_simul_vec = zeros(Ntot,LengthSIM); % consumption vector by bin and periods
                Ut_simul_vec = u_c(conso_simul_vec) - v_l(l_simul_vec) -0.5*psiw*piw_simul_vec.*piw_simul_vec ;
                Ut_simul_vec = repmat(S_h,1,LengthSIM).*Ut_simul_vec;
                W = Ut_simul_vec(:,end);
                ...
                Welfare(i-1, Economy,:) = (1-beta)*W./S_h; % percapita
    ...
    CE1 = zeros(Maxseed,Ntot);
    CE1 = 100*(exp(Welfare(:,1,:)-Welfare(:,4,:))-1); % per capita

    CEav1 = zeros(Maxseed,2); 

    for i =1:Maxseed
        CEav1(i,1) =(sum(CE1(i,:)'.*S_h));
    end


    meanCE1 =  zeros(Ntot,1);

    for i=1:Ntot
        meanCE1(i,1) = mean(CE1(:,i));
    end


    for i=1:eco.ny
        index = find(trunc.y0_h == eco.ys(i));
        CEy1(index) = real(sum(S_h(index).*meanCE1(index,1))/sum(S_h(index)));
    end


    av1 = mean(CEav1)

    state_idio = trunc.y0_h; 

    save("For_Welfare_SW.mat", "CEy1", "state_idio","av1") 
