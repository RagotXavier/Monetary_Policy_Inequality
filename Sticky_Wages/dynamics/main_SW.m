...
for Economy = [1, 4]
    ...
    file = ['code_dynare_SW',num2str(Economy),'.mod'];
    fid = fopen(file, 'w') ;
    formatSpec = '%25.20f';
    ...
    var_mom = diag(var_mom);
    std = var_mom.^(1/2);
    std_norm = (std./mean_mom)*100;
    std_norm = round(std_norm,3);
    mean_var = round(mean_mom,3);
    ...
    corr_pip_y = var_init(index_Y,index_pip)./( std(index_Y)*std(index_pip) ) ;% Y, pip
    ...
    baseFileName_momemt = sprintf('moments_eco%d', Economy);
...
save("To_IRFs_SW.mat", "RBv", "Kv", "Tv","PIpv", "PIwv", "Yv", "Bv", "Cv", "rv", "taulv", "taukv", "Lv", "Uv", "LengthIRF") 
